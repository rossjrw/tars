...
def process_links(command_infos, other_texts):
    ...
    Available commands:
        @repo(name) - Links to the configured repository with the given link
        text.
        @example(text) - Formats the text as a command example.
        @example(text)(description) - Formats the text as a command example
        with an explanation. Must be in a block by itself.
        @command(alias) - Links to the given command. The string provided may
        be any alias and may start with nothing, `.`, or `..`.
        @argument(name) - Links to the argument with the given name in the
        current command. Can only be used in command or argument documentation,
        not other text.
        @section(name) - Links to the section with the given name.
...
def replace_block_example(string, **_):
    ...
    def replace(match):
        return "**Example:** {} &mdash; {}".format(
            replace_inline_example(match.group(1)), match.group(2)
        )

    return re.sub(r"(?<=\n)(@example\([^)]*\))\(([^)]*)\)", replace, string)


def replace_inline_example(string, **_):
    """Replaces inline examples."""

    def replace(match):
        return "`{}`".format(match.group(1))

    return re.sub(r"@example\(([^)]*)\)", replace, string)
